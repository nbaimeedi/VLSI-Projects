...
#set padRing           180.0
# space for power ring
#set coreMargin [expr $padRing + 20] 
set coreMargin 25
...
initialize_floorplan -die_area "0 0 $chipW $chipH" \
                     ...
                     -site "CoreSite"
#save_checkpoint 00_${proj_name}.initialize_floorplan

utl::report "Connect global nets (power)"
source scripts/power_connect.tcl
#save_checkpoint 01_${proj_name}.power_connect

utl::report "Create Floorplan"
source scripts/floorplan.tcl
#save_checkpoint 02_${proj_name}.floorplan

utl::report "Create Power Grid"
source scripts/power_grid.tcl
#save_checkpoint 03_${proj_name}.power_grid
##report_image "00_${proj_name}.power" true
#
#################################################################################
## Pin Placement								      #
#################################################################################
utl::report "Pin Placement"
##set port [get_ports]
##puts $port
##foreach port [get_ports *] {
##	set dir [get_port_direction $port]
##	if { $dir == "input" } {
##		lappend inputs $port
##	} elseif { $dir == "output" } {
##		lappend outputs $port
##	} elseif { $dir == "inout" } {
##		lappend inouts $port
##	}
##}
...
##	set_io_pin_constraint -pin_pattern $inputs -region left
##}
##if {[info exists outputs]} {
##	set_io_pin_constraint -pin_pattern $outputs -region right
##}
##if {[info exists inouts]} {
##	set_io_pin_constraint -pin_pattern $inouts -region bottom
set_io_pin_constraint -direction input -region left:*
set_io_pin_constraint -direction output -region right:*
set_io_pin_constraint -direction inout -region left:*
place_pins -hor_layers Metal2 -ver_layers Metal1 -min_distance 2.5 -corner_avoidance 20
##save_checkpoint 04_${proj_name}.pin_placement
#
#################################################################################
...
#save_checkpoint 05_${proj_name}.pre_place
...
# Set maximum routing layer to be Metal5 for CPU (sub-module)
...
#save_checkpoint 06_${proj_name}.gpl1
...
#save_checkpoint 06_${proj_name}.gpl1_repaired
...
#save_checkpoint 07_${proj_name}.gpl2
...
#save_checkpoint 08_${proj_name}.dpl
...
#save_checkpoint 09_${proj_name}.cts
...
set_global_routing_layer_adjustment Metal5 0.20
set_routing_layers -signal Metal1-Metal5 -clock Metal1-Metal5
...
#save_checkpoint 10_${proj_name}.g_route
...
#save_checkpoint 11_${proj_name}.d_route
...
save_checkpoint 12_${proj_name}.final
...
report_metrics "12_${proj_name}_metrics.final"
