...
#}
#
#proc cve2_core_power { name macro } {
#	global mprWidth mprSpacing mprOffsetX mprOffsetY mpgWidth mpgSpacing mpgOffset
#	# Macro Grid and Rings
#	define_pdn_grid -macro -cells $macro -name ${name}_grid -orient "R0 R180 MY MX" \
#		-grid_over_boundary -voltage_domains {CORE} \
...
#		-add_connect
#
#	set cpu [[ord::get_db] findMaster $macro]
#	set cpuHeight [ord::dbu_to_microns [$cpu getHeight]]
#	set stripe_dist [expr $cpuHeight - 2*$mpgOffset - $mpgWidth - $mpgSpacing]
#	utl::report "stripe_dist of $macro: $stripe_dist"
#
#	# for the large macros there is enough space for an additional stripe
#	if {$stripe_dist > 100} {
...
#}
#
#proc timer_unit_power { name macro } {
#	global mprWidth mprSpacing mprOffsetX mprOffsetY mpgWidth mpgSpacing mpgOffset
#	# Macro Grid and Rings
#	define_pdn_grid -macro -cells $macro -name ${name}_grid -orient "R0 R180 MY MX" \
#		-grid_over_boundary -voltage_domains {CORE} \
...
#		-add_connect
#
#	set timer [[ord::get_db] findMaster $macro]
#	set timerHeight [ord::dbu_to_microns [$timer getHeight]]
#	set stripe_dist [expr $timerHeight - 2*$mpgOffset - $mpgWidth - $mpgSpacing]
#	utl::report "stripe_dist of $macro: $stripe_dist"
#
#	# for the large macros there is enough space for an additional stripe
#	if {$stripe_dist > 100} {
...
#		-add_connect
#
#	set debug [[ord::get_db] findMaster $macro]
#	set debugHeight [ord::dbu_to_microns [$debug getHeight]]
#	set stripe_dist [expr $debugHeight - 2*$mpgOffset - $mpgWidth - $mpgSpacing]
#	utl::report "stripe_dist of $macro: $stripe_dist"
#
#	# for the large macros there is enough space for an additional stripe
#	if {$stripe_dist > 100} {
...
##add_pdn_stripe -grid {core2_grid} -layer {Metal3} -width {0.44} -offset {0} \
#               -followpins -extend_to_core_ring
#
##get_nets
##add_pdn_stripe -grid {core2_grid} -layer {Metal2} -width {4} -offset {24} \
##               -spacing 14 -pitch 28 -followpins -nets {VSS VDD} -snap_to_grid
#	       
##add_pdn_stripe -layer Metal2 -followpins -width 2
#sram_power "sram_256x64"  "RM_IHPSG13_1P_256x64_c2_bm_bist"
#cve2_core_power "cve2_core" "cve2_core"
#dm_top_power "dm_top" "dm_top"
#timer_unit_power "timer_unit" "timer_unit"
#
## Top power grid
## Top 2 Stripe
#add_pdn_stripe -grid {core2_grid} -layer {TopMetal2} -width $tpg2Width \
#               -pitch $tpg2Pitch -spacing $tpg2Spacing -offset $tpg2Offset \
#               -extend_to_core_ring -snap_to_grid -number_of_straps 14
#
...
##add_pdn_connect -grid {core2_grid} -layers {TopMetal2 Metal3}
#add_pdn_connect -grid {core2_grid} -layers {TopMetal2 Metal2}
#add_pdn_connect -grid {core2_grid} -layers {TopMetal2 Metal4}
## add_pdn_connect -grid {core2_grid} -layers {TopMetal2 TopMetal1}
## power ring to standard cell rails
##add_pdn_connect -grid {core2_grid} -layers {Metal3 Metal1}
#add_pdn_connect -grid {core2_grid} -layers {Metal3 Metal2}
#
#
###########################################################################
###  Generate
