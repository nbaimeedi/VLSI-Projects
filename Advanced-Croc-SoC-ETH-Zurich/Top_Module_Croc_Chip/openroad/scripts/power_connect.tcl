...
add_global_connection -net {VSS} -inst_pattern {.*} -pin_pattern {vdd} -ground
# pads
add_global_connection -net {VDD} -inst_pattern {.*} -pin_pattern {vdd} -power
...
# Sub-module cve2_core, dm_top and timer_unit
add_global_connection -net {VDD} -inst_pattern {.*} -pin_pattern {VDD} -power
add_global_connection -net {VSS} -inst_pattern {.*} -pin_pattern {VSS} -ground
add_global_connection -net {VSS} -inst_pattern {.*} -pin_pattern {VSSIO} -ground
add_global_connection -net {VDD} -inst_pattern {.*} -pin_pattern {VDDIO} -power
...
report_net -connections VDD
report_net -connections VSS
# standard cell grid and rings
define_pdn_grid -name {core2_grid} -voltage_domains {CORE}
